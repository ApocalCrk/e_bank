...
class Tabungan extends CI_Controller
{
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $cek = $this->db->query("SELECT * FROM tabungan where nis='$nis'");
            if ($cek->num_rows() != 0) {
                    $saldo_tambahan = $this->input->post('saldo',TRUE);
                    $waktu = date('d-m-Y').' '.$jam;
                $this->db->query("UPDATE tabungan SET saldo=saldo+'$saldo_tambahan',saldo_tambahan='$saldo_tambahan',waktu='$waktu' WHERE nis='$nis'");
                $data_detail = array(
                    'nis' => $nis,
                    'saldo_tambahan' => $saldo_tambahan,
                    'waktu' => $waktu,
                );
                $this->db->insert('detail_tambahan',$data_detail);
    ...
    public function tambah_saldo() 
    {
        ...
        'pengeluaran' => set_value('pengeluaran'),
        'nis' => set_value('nis'),
        'konten' => 'tabungan/tabungan_form_tambah_saldo',
            'jdl' => 'Tambah Saldo Siswa',
    );
        $this->load->view('admin_access/v_index', $data);
    }
    
    public function tambah_saldo_action() 
    {
        $this->_rules();
